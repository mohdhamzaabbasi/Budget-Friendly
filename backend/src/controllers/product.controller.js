...
const createProduct = asyncHandler(async (req, res) => {
  ...
  const minimumQuantityValue = Number(minimumQuantity);

  if (!isWholeNumber(minimumQuantity) || minimumQuantityValue < 0) {
    return res.status(400).json({ message: "Minimum quantity must be a whole number" });
  }
  ...
  const product = await Product.create({
    ...
    saleRate: 0,
    quantity: 0,
    minimumQuantity: minimumQuantityValue,
    batch: 0,
    isActive: true
  });
...
const updateProduct = asyncHandler(async (req, res) => {
  ...
  if (
    Object.prototype.hasOwnProperty.call(updates, "minimumQuantity") &&
    (!isWholeNumber(updates.minimumQuantity) || Number(updates.minimumQuantity) < 0)
  ) {
    return res.status(400).json({ message: "Minimum quantity must be a whole number" });
  }

  if (Object.prototype.hasOwnProperty.call(updates, "minimumQuantity")) {
    updates.minimumQuantity = Number(updates.minimumQuantity);
  }
