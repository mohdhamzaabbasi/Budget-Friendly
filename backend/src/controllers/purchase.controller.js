...
const createPurchase = asyncHandler(async (req, res) => {
  ...
  const result = await runWithOptionalTransaction(async (session) => {
    ...
    for (const item of items) {
      ...
      if (!productCode || purchaseRate < 0 || saleRate < 0 || !isWholeNumber(quantity) || quantity < 1) {
        const error = new Error("Each purchase item needs product, purchase rate, sale rate, and whole-number quantity");
